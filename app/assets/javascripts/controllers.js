...
.directive('reply', ['$compile', function($compile) {
  return {
    ...
    template: '<li>' +
                '<span class="author"><avatar author="reply.author"></avatar> <a ng-href="https://www.laarc.io/user?id={{ reply.author }}" ng-title="See {{ reply.author }} profile">{{reply.author || "deleted"}}</a></span><span ng-if="reply.created_at"> - <a class="date" ng-href="https://www.laarc.io/item?id={{reply.story_id}}#{{reply.id}}" title="{{ reply.created_at }}">{{reply.created_at_i | moment:"M/D/YYYY h:m A"}}</a></span>' +
